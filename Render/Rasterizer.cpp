...
namespace drawing{
    ...
    void Rasterizer::rasterizeAsync(rendering::World *world, rendering::Camera * camera, RenderCallback  callBack) {
            using namespace geometry;
            resetZBuffer();
       
            for(int object_id = 0; object_id < world->objects().size(); object_id++){
                data::Object object = world->objects()[object_id];
                dispatch_apply(object.mesh().triangleIndexes().size(), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(size_t triangle_id){
                    ...
                    for(; y <= int(pC.y()); y++){
                        ...
                        for(int x = int(xBegin); x <= int(xEnd); x++){
                            ...
                            if (valid(x, y) && zbuffer(x, y) <  pA.depth() * baricentric(0) + pB.depth() * baricentric(1) + pC.depth() * baricentric(2)) {
                                continue;
                            }
